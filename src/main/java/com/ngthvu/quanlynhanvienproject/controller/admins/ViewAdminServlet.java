...
public class ViewAdminServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        Integer start = 0;
        if(pageNum > 1) {
            start = (pageNum-1)*NUMBER_ADMIN_PER_PAGE+1;
        }
        else{
            start = pageNum;
        }
        ...
        long endCount = startCount + NUMBER_ADMIN_PER_PAGE -1;
        ...
        request.getRequestDispatcher("/view/admins/admin_list.jsp").forward(request,response);
    }
